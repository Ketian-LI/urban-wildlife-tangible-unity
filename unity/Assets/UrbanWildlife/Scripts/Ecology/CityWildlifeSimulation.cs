...
namespace UrbanWildlife.Ecology
{
    public sealed class CityWildlifeSimulation
    {
            ...
            float minimumUtility = 0.45f)
        {
            ...
            CityWildlifeAgent immigrant = CreateAgent(species, sequence, best.position_norm);
        ...
        private void SpawnInitial(CityWildlifeSpecies species, int count)
        {
            ...
            {
                CityGreenPatch patch = patches[(index + (int)species) % patches.Length];
                float[] centre = PolygonCentroid(patch.polygon_norm);
                float offset = ((index % 3) - 1) * 0.008f;
            ...
            CityEnvironmentSnapshot environment)
        {
            ...
            // Deterministic axis detour keeps agents out of building footprints.
        ...
        private bool IsBlocked(float[] point)
        {
            ...
            foreach (CityBuilding building in city.buildings ?? Array.Empty<CityBuilding>())
            {
                ...
                if (Mathf.Abs(point[0] - building.position_norm[0]) <= halfWidth &&
                    Mathf.Abs(point[1] - building.position_norm[1]) <= halfHeight)
                {
                    return true;
                }
            }
