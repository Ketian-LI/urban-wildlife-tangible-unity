...
namespace UrbanWildlife.Networks
{
    public static class CityRoadCandidateGenerator
    {
        ...
        public const float AutomaticAccessSidewalkOffsetUnits = 0.70f;
        public const float AutomaticAccessSidewalkWidthUnits = 0.42f;
        ...
        {
            ValidateInputs(city, building);
            CityRoadCandidate roadCandidate = Generate(city, building).candidates
                .First(candidate => candidate.route_option == CityRoadRouteOption.Direct);
            return CreateBasicPedestrianAccess(city, building, roadCandidate);
        }

        public static CityPedestrianLink CreateBasicPedestrianAccess(
            CityState city,
            CityBuilding building,
            CityRoadCandidate roadCandidate)
        {
            ValidateInputs(city, building);
            if (roadCandidate?.points_norm == null || roadCandidate.points_norm.Length < 2)
            {
                throw new ArgumentException(
                    "A selected vehicle access route is required to generate its sidewalk.",
                    nameof(roadCandidate));
            }

            float[][] leftSide = OffsetPolyline(
                roadCandidate.points_norm,
                city.bounds,
                AutomaticAccessSidewalkOffsetUnits);
            float[][] rightSide = OffsetPolyline(
                roadCandidate.points_norm,
                city.bounds,
                -AutomaticAccessSidewalkOffsetUnits);
            NetworkConnection leftConnection = ClosestConnection(
                leftSide[leftSide.Length - 1],
                city.pedestrian_links,
                city.bounds,
                false);
            NetworkConnection rightConnection = ClosestConnection(
                rightSide[rightSide.Length - 1],
                city.pedestrian_links,
                city.bounds,
                false);
            bool useLeftSide = leftConnection.DistanceUnits <= rightConnection.DistanceUnits;
            float[][] sidePoints = useLeftSide ? leftSide : rightSide;
            NetworkConnection connection = useLeftSide ? leftConnection : rightConnection;
            List<float[]> points = sidePoints.Select(ClonePoint).ToList();
            if (Distance(points[points.Count - 1], connection.Point, city.bounds) > 0.01f)
            {
                points.Add(ClonePoint(connection.Point));
            }
            return new CityPedestrianLink
            {
                ...
                points_norm = points.ToArray(),
                width_units = AutomaticAccessSidewalkWidthUnits,
        ...
        public static float[][] OffsetPolyline(
            IEnumerable<float[]> source,
            CityBounds bounds,
            float offsetUnits)
        {
            float[][] points = (source ?? Array.Empty<float[]>())
            ...
            {
                throw new ArgumentException(
                    "A valid city bounds and a polyline with at least two points are required.");
            }

            float[][] result = new float[points.Length][];
            for (int index = 0; index < points.Length; index += 1)
            {
                float[] previous = points[Math.Max(0, index - 1)];
                float[] next = points[Math.Min(points.Length - 1, index + 1)];
                float tangentX = (next[0] - previous[0]) * bounds.width_units;
                float tangentY = (next[1] - previous[1]) * bounds.height_units;
                ...
                float normalX = -tangentY / tangentLength;
                float normalY = tangentX / tangentLength;
                float worldX = points[index][0] * bounds.width_units + normalX * offsetUnits;
                float worldY = points[index][1] * bounds.height_units + normalY * offsetUnits;
                result[index] = new[]
                {
                    Clamp(worldX / bounds.width_units, 0f, 1f),
                    Clamp(worldY / bounds.height_units, 0f, 1f),
                };
            }
            return result;
        }
