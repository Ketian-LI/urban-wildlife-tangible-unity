...
- `CityPrototypeDemo`的桌面建造预览继续使用连续地图坐标，并且只显示建筑真实旋转占地与半透明建筑Sprite：绿色可建、红色冲突。18 × 12后台单元不再画到预览层，避免跨格时误读为扩大占地；Desktop碰撞也只读取建筑真实几何，两个不相交的建筑即使覆盖同一个生态统计格也可分别登记、分别拆除。确认后，林地转暖白的范围仍只覆盖建筑真实占地和窄幅接入道路。无效红色预览可由下一次地图点击直接替换，不需要先取消；有效绿色预览仍必须明确确认或取消。该重试规则与共享后台格规则仅用于Desktop输入，Camera扫描流程保持不变。
