...
6. Run显示60秒倒计时，随后自动进入30秒Observe；三个周期结束后可用 **RESET SESSION** 重新开始。
   Run中会出现2个Walker、2个Dweller和2个Visitor：三类人物分别播放6张独立行走Sprite；所有行走帧都按同一人物高度和脚底基线归一化，因此不会在相邻帧之间忽大忽小或上下闪动。Dweller在广场停留，以4张独立Sprite坐下并反向播放起身；Visitor绕行至Food Hotspot并播放6张独立喂食Sprite。人物表现层采用8 FPS离散关键帧：3帧待机、3帧转身、6帧行走、6帧喂食、4帧坐下和4帧起身。Run与Observe期间控制面板自动收起为紧凑信息卡，减少对地图的遮挡。
...
- Unity已完成可测试的逐帧动画节奏样板与角色标志性动作：三类人物和三种动物各有6帧独立行走，三种动物各有6帧进食，Dweller有4帧坐下并可反向起身，Visitor有6帧喂食。待机与转身仍使用共用离散关键帧；拥堵寻路和Trace尚未实现。
