...
- Automatically generated building access uses the same warm-white kerb, pale-grey surface, centre dashes and total width as the baked original streets. The 1.05-unit cross-section is divided into a 0.73-unit grey vehicle lane and two 0.16-unit white pedestrian edges; the edges sit inside the shared road width rather than widening it. Agents still follow a separate side route.
- Vehicle routes begin and end beyond the building footprint with enough clearance for the car artwork, so outbound and returning cars remain on the access road without overlapping the building sprite.
