...
移动元素使用相对于建筑占地的独立显示标尺：开局独栋使用0.72表现系数，透明边距收紧后在1536 × 1024截图中约45–50 px宽；公寓、市场和社区分别保持1.05、1.00与0.88表现系数。人物高度与车辆进深均约0.24单位，四种动物宽度保持在0.15–0.28单位。Sprite缩放只发生在`CityPrototypeDemo`表现层；真实5 × 5、7 × 7、7.5 × 5和9 × 6占地、碰撞与清林范围不随贴图显示缩放。`CityTripAgent`和`CityWildlifeSimulation`仍输出原坐标、速度和状态，因此缩小画面角色不会改变车辆沿路中心行驶、行人走侧边步道、动物热点聚合或生态结果。
...
- `CityPrototypeDemo`的桌面建造预览继续使用连续地图坐标；18 × 12网格只以低对比线框提示受影响单元，不承担吸附。建筑真实旋转占地以绿色（可建）或红色（冲突）显示，并叠加半透明建筑Sprite；确认后，林地转暖白的范围仍只覆盖建筑真实占地和窄幅接入道路。
