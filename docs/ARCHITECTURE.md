...
`CityNetworkPlanningManager` 在Route Selection Preview中要求每栋建筑选且只选一个机动车候选。确认后把选择结果写为`BuildingAccess` VehicleRoad，并更新Building引用。自动接入道路与底图原始道路使用同一视觉规格：完整横断面1.05单位、暖白路缘、按底图实机像素重取的浅灰路面、灰色中心虚线和一层低透明柔边；主路底图保留原虚线，三个既有支路由运行时补齐同规格虚线，因此整张路网不再混用“有标线/无标线”两套规则。候选路线读取目标道路局部切线，使末段以近似法线方向接入，中心虚线在交点前停止，接入端再使用无圆帽线段和浅灰融合面消除接缝，形成连续丁字路口。支路造成的暖白清林带只绘制在路线实际穿过`was_woodland`后台单元的连续区段，开放地不再产生多余白色胶囊。Pedestrian Network仍使用独立数据数组，但几何从车辆道路派生：自动接入路把1.05单位总宽分为0.73单位灰色车道和两侧各0.16单位白色步行区域，实际行人路线继续选择其中一侧，避免视觉上出现一侧宽、一侧窄，也不会在旧路之外额外拓宽；玩家改选Direct、Existing-network或Low-impact时道路和行人路线同步重算。玩家仍可在屏幕中新增、调整或删除`ScreenEdited ExtraFootpath`。确认是原子事务，删除ExtraFootpath时同步移除建筑引用；任何State验证失败都保留上一版CityState。正式输入中没有机动车或步行彩带字段。
